...
## Pipeline Structure
```{r}
NAME <- '2_hindi_clustering_langexp' ## Name of the R file (w/o file extension!)
PHASE <- 'P2-langexp-data' ## Name of the project phase 
PROJECT <- 'langx' ## Name of project

if (dir.exists(file.path('..', '..', '002-pipeline'))){
  pipeline <- file.path('..', '..', '002-pipeline', PHASE, NAME)
} else {
  pipeline <- file.path('..', '..', '002-pipeline', PHASE, NAME)
}
...
#### Load in Data
```{r}
processing_out <- file.path('..', '..', '002-pipeline', PHASE, "1_hindi_processing", "out")

load(file=file.path(processing_out, "hindi.RData"))
load(file=file.path(processing_out, "hindi_subj.RData"))
load(file=file.path(processing_out, "hindi_groups.RData"))
```
...
#              habillage = hindi_mdist$outlier,  palette = "jco",
...
# Specify file properties (must occur before) 
## NOTE: Saved in 'out'
png(filename = file.path(pipeline, "temp", "langexp_clust4_dendextend.png"), res = 300, width = 12, height = 5, units = 'in')
...
#   select(-clusters_2, -clusters_3) %>%
  # save(file = file.path(pipeline, "store", "langexp_final_data.RData"))
```
...
fviz_cluster(list(data = langexp_scaled, cluster = clusters_4),
             ...
             palette = "jco") + gg_theme() + 
  labs(fill = "Cluster", color = "Cluster", shape = "Cluster")
# ggsave(filename = file.path(pipeline, "temp", "langexp_clust4_cluster.png"))
...
print(xtable::xtable(cluster_means_4, type = "latex"), file = file.path(pipeline, "temp", "clustermeans.tex"))
...
# print(xtable::xtable(cluster_zmeans_4, type = "latex"), file = file.path(pipeline, "temp", "clusterzmeans.tex"))
...
ggsave(filename = file.path(pipeline, "temp", "langexp_clust4_means.png"), width=12, height=7)
...
# sil <- silhouette(clusters_3, dist(langexp_scaled))
# fviz_silhouette(sil, palette="jco") + theme_bw()
```
...
# Save HTML, PNG, and TEX files of table
gtsave(cluster_counts, file.path(pipeline, "temp", "langexp_clust4_counts.html"))
webshot2::webshot(file.path(pipeline, "temp", "langexp_clust4_counts.html"), file.path(pipeline, "temp", "langexp_clust4_counts.png"), vwidth = 625, zoom = 2.5)
gtsave(cluster_counts, file.path(pipeline, "temp", "langexp_clust4_counts.tex"))
